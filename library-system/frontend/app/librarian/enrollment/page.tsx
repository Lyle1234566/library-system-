...
import { enrollmentApi, type EnrollmentImportResult, type EnrollmentSummary } from '@/lib/api';
...
const TEMPLATE_CSV = `student_id,full_name,school_email,program,year_level,academic_term,is_currently_enrolled,notes
S-2026-0001,Jane Student,jane.student@school.edu,BSIT,3,2025-2026,TRUE,Verified by registrar
S-2026-0002,John Student,john.student@school.edu,BSCS,2,2025-2026,FALSE,Leave inactive when not enrolled
`;
...
export default function LibrarianEnrollmentPage() {
  ...
  const [uploadResult, setUploadResult] = useState<EnrollmentImportResult | null>(null);

  const applySummaryResponse = useCallback((response: Awaited<ReturnType<typeof enrollmentApi.getSummary>>) => {
    if (response.error || !response.data) {
      setSummary(null);
      setSummaryError(response.error ?? 'Unable to load enrollment summary.');
  ...
  const loadSummary = useCallback(async () => {
    ...
    applySummaryResponse(await enrollmentApi.getSummary());
  ...
  useEffect(() => {
    ...
    const fetchInitialSummary = async () => {
      const response = await enrollmentApi.getSummary();
      if (!cancelled) {
  ...
  const downloadTemplate = () => {
    ...
    anchor.href = url;
    anchor.download = 'enrollment-template.csv';
    anchor.click();
  ...
  const handleUpload = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await enrollmentApi.importCsv(selectedFile, academicTerm);
    setIsUploading(false);

    if (response.error || !response.data) {
      setUploadError(response.error ?? 'Unable to upload the enrollment CSV.');
      return;
    }
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-semibold text-white sm:text-4xl">
                Upload student enrollment CSV
              </h1>
              <p className="mt-3 max-w-3xl text-sm leading-6 text-white/65 sm:text-base">
                Keep student registration tied to the current registrar list. Upload a CSV here and the
                register page will only allow enrolled students to continue.
              </p>
            </div>
            ...
            <div className="rounded-[32px] border border-white/12 bg-white/[0.05] p-6 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-8">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-semibold text-white">Import the current student list</h2>
                  <p className="mt-2 max-w-2xl text-sm text-white/65">
                    The CSV must include a `student_id` column. Existing rows are updated by student ID,
                ...
                <div className="rounded-[28px] border border-dashed border-sky-300/25 bg-sky-400/[0.05] p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <label className="text-sm font-semibold text-white">Enrollment CSV file</label>
                      <p className="mt-1 text-sm text-white/60">
                        Accepted format: `.csv`, UTF-8 encoded, with one row per student.
                      </p>
                      <input
                        type="file"
                        accept=".csv,text/csv"
                  ...
                  <div className="rounded-[28px] border border-white/10 bg-[#0b1729]/88 p-5">
                    ...
                    <div className="mt-3 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-white/60">`student_id`</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-[32px] border border-white/12 bg-white/[0.05] p-6 shadow-2xl shadow-black/30 backdrop-blur-xl">
                ...
                <p className="mt-5 text-sm leading-6 text-white/60">
                  Rows missing `student_id` are skipped. The upload updates matching student IDs instead of
                  creating duplicates.
                </p>
              </div>
            </div>
