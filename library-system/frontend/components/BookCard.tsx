...
export default function BookCard({ book, readOnly = false }: BookCardProps) {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 overflow-hidden rounded-[18px]">
        <div className="absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          <div
            ...
            style={{
              background: 'linear-gradient(90deg, #8edbff, transparent)',
      ...
      <div className="absolute inset-0 z-10 rounded-[18px] bg-[#060e24]/20 backdrop-blur-[50px] transition-opacity duration-500 group-hover:opacity-0" />

      <div className="relative z-20 flex h-full w-full flex-col rounded-[18px] border border-white/15 bg-[#10263d] p-2 text-white shadow-[0_0_3px_1px_rgba(0,0,0,0.53)] transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-[0_16px_30px_rgba(0,0,0,0.4)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-300 focus-visible:ring-offset-2 focus-visible:ring-offset-[#071825]">
        <div className="absolute inset-0 rounded-[18px] opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          <div className="absolute left-1/2 top-1/2 h-[50px] w-[50px] -translate-x-1/2 -translate-y-1/2 bg-sky-200/25 blur-[50px]" />
        </div>
        ...
        <div className="relative flex h-full flex-col">
                ...
                >
                  <span
                    className={`h-1.5 w-1.5 rounded-full ${
                      localBook.available ? 'bg-sky-300' : 'bg-slate-300'
                    }`}
                  />
                  {availabilityLabel}
                </span>
          ...
          <div className="mt-2 flex items-center justify-between px-0.5 text-[11px]">
            ...
            <span className="font-medium text-sky-200">{publishedYear}</span>
          </div>
          ...
          <div className="mt-auto pt-2.5">
            {readOnly ? (
              <Link
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-[10px] border border-amber-300/40 bg-amber-500/10 px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-amber-200 transition hover:bg-amber-500/20"
              >
                Sign in to Borrow
              </Link>
            ) : (
              <button
                ...
                className={`
                  ...
                  transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-300
                  focus-visible:ring-offset-2 focus-visible:ring-offset-[#071825]
            ...
            {!readOnly && (borrowState.error || borrowState.message) && (
              <div
